...
SparseMatrix layMine(int row, int col, int rows, int cols, SparseMatrix board);
...
int main(int argc, char* argv[]) {
    ...
    int minesLaid = 0;
    std::vector<std::pair<int, int>> mineCoords;
    while (minesLaid < mines) {
        int tryRow = rand()%rows;
        int tryCol = rand()%cols;
        std::pair<int, int> tryCoords = std::make_pair(tryRow, tryCol);
        if (!(std::find(mineCoords.begin(), mineCoords.end(), tryCoords) != mineCoords.end())) {
            // Lay a mine here
            board = layMine(tryRow, tryCol, rows, cols, board);
            mineCoords.push_back(tryCoords);
            minesLaid++;
        }
    }
